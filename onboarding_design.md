...
- ✅ **Rendered / shareable export (capability "D") — built.** Producing a polished, self-contained document to hand to someone *outside* the system is now the **`export`** skill (`.skills/export/SKILL.md`) over the **`reports/render_html.py`** engine: it renders any A3 or Kaizen to consistently-structured, self-contained HTML under `reports/` (every A3 the same skeleton, every Kaizen the same skeleton; missing sections become labelled placeholders). It landed as the first registry module — own `export` skill, `capabilities: { export: { enabled: true } }` config entry, output to `reports/` (gitignored, outside `data/`), and `review`'s reserved "export this" hand-off now points at it. It came in **stdlib-only** (HTML needs no third-party library), so the §5.4 optional-dependency machinery isn't exercised yet; a heavier export (PDF, slides) would be the case that exercises it. *(Built via the meta-system as a feature-addition run; see `meta-system/logs/`.)*
