...
- ✅ **Rendered / shareable export (capability "D") — built.** Producing a polished, self-contained, **management-facing** document is now the **`export`** skill (`.skills/export/SKILL.md`) over the **`reports/render_html.py`** engine. It renders A3s, Kaizens, and combined per-investigation **bundles** to self-contained HTML under `reports/` as an executive synthesis under fixed headings — **The situation → What we found → What we did → Where it stands** — that strips systems jargon (file paths, artifact IDs, calc/bash commands, internal refs), humanises metric variable names (`cph` → throughput), and draws an inline **SVG** trend chart of the driving metric from the canonical metric CSVs (baseline/dip/recovery, target + signal date marked). Bundles frame the immediate facility fix vs the systemic fix. It landed as the first registry module — own `export` skill, `capabilities: { export: { enabled: true } }` config entry, output to `reports/` (gitignored, outside `data/`), and `review`'s reserved "export this" hand-off now points at it. It came in **stdlib-only** (HTML + SVG need no third-party library), so the §5.4 optional-dependency machinery isn't exercised yet; a heavier export (PDF, slides) would be the case that exercises it. *(Built via the meta-system as a feature-addition run; see `meta-system/logs/`.)*
