...
7. **Verify, then flip the mode.** Run `bash verify.sh` (the structural checks; the demo-scenario assertions in Sections 5–11 are demo-specific and expected to fail once demo data is gone — see the note below). Only when the structural checks pass do you set the mode: `python config/deployment.py set production`.
...
> **Note on `verify.sh` after a production cutover.** Sections 5–11 assert demo-specific scenarios (the dal-02 dip, chr-05, the equipment pattern, etc.) and will fail once demo data is gone — that's expected. A production-aware split of `verify.sh` (structural checks vs demo-scenario assertions) is a planned onboarding slice; until it lands, treat the structural checks (golden tests, reconcile, validators, calc smoke) as the acceptance gate and the demo-scenario failures as expected.
