...
```yaml
current_phase:          "Portfolio core + Phases 5-6 closeout + production-onboarding (slices 1-6) + a REVIEW/REPORTING access layer — all complete. Phase 5: 3 outcome calcs (follow_up_check, countermeasure_effectiveness, intervention_attribution); 5 closed investigations (1 A3 + 3 Kaizens). Phase 6: add_playbook procedure + playbook template (maintain 6 of 9) + the first pattern. Review layer: read-only `review` skill + status.py + consistent catalogs (kaizens INDEX gap filled) + standardized morning brief. Remaining: the shareable-export capability ('D'), flagged CRITICAL."
last_completed_step:    "Built the review/reporting access layer (the 'not easily shown/accessed' gap) ON TOP OF the parallel Phase 5/6 closeout (outcome calcs + add_playbook), then rebased and integrated the two cleanly. Review layer: .skills/review/SKILL.md (6th skill) + .skills/review/status.py (consistent dashboard/category/open/due renderer) + data/kaizens/INDEX.md (missing catalog; open_kaizen now maintains it); signal-detect morning brief standardized to a fixed format reusing status.py; reset_demo_state fixed to preserve+reset kaizens/INDEX.md. verify.sh Section 13. See working-log Sessions 17/18 (parallel Phase 5/6 + Codex fixes) and 19 (this — review layer + integration). Counts re-verified after the rebase."
next_concrete_action:   "Build capability 'D' — rendered/shareable exports (investigation+A3/Kaizen+outcomes → one report/slide/PDF) as the first true capability-registry module (own setup, capabilities: entry, optional deps w/ graceful degrade, output to reports/). review already has the 'export this' hand-off reserved. Flagged critical: the operator needs to express work to others."
in_progress_work:       null
blocked_on:             null
last_updated:           "2026-05-21"
last_updated_by:        "session-2026-05-21-review-capability"
sessions_logged:        19
```
...
| Phase 4 — signal-detect + investigate + first playbook | in progress (exit criteria met for the demo) | [throughput_drop.md](.skills/investigate/playbooks/throughput_drop.md) (now consults patterns) + [damage_spike.md](.skills/investigate/playbooks/damage_spike.md) playbooks; **5 investigations** across throughput_drop (×4) + damage_spike (×1) | 2 playbooks; 5 investigations closed end-to-end. chr-03 was run by **Codex operating the system live**. Playbooks authored from real investigations (plan 4.6). throughput_drop now has a pattern-check Step 0. |
| Phase 5 — close-loop + first A3/Kaizen + outcome calcs | complete | [k-2026-05-dal-02-trainer-ratio](data/kaizens/open/k-2026-05-dal-02-trainer-ratio.md) + [k-2026-05-chr-03-bin-relocation](data/kaizens/open/k-2026-05-chr-03-bin-relocation.md) + [k-2026-05-ral-02-conveyor-pm](data/kaizens/open/k-2026-05-ral-02-conveyor-pm.md) Kaizens; [a3-2026-05-network-trainer-coverage](data/a3s/open/a3-2026-05-network-trainer-coverage.md) A3; outcome calcs **3 of 3** ([follow_up_check](calc/outcome/follow_up_check.sh), [countermeasure_effectiveness](calc/outcome/countermeasure_effectiveness.sh), [intervention_attribution](calc/outcome/intervention_attribution.sh)), all golden-tested; [.skills/close-loop/procedures/](.skills/close-loop/procedures/) (open_kaizen, open_a3, reopen_investigation) | **Complete (2026-05-20).** 5 closed investigations (1 A3 + 3 Kaizens); all 3 outcome calcs built + golden-tested. Deviation: the plan's 6 close-loop procedures were consolidated to 3 — close/escalate are handled inline in the SKILL and log_floor_findings is the intake walk itself; close_a3/close_kaizen are driven by the outcome calcs + follow_ups INDEX. Logged in Decisions. |
| Phase 6 — Pattern emergence + maintain skill | complete | [.skills/maintain/procedures/](.skills/maintain/procedures/) (add_calc, add_pattern, update_pattern, **add_playbook**), [pattern.md](.skills/maintain/templates/pattern.md) + [playbook.md](.skills/maintain/templates/playbook.md) templates, [patterns/equipment_downtime_throughput_drag.md](data/patterns/equipment_downtime_throughput_drag.md) + [patterns/INDEX.md](data/patterns/INDEX.md) | **Complete (2026-05-20):** all exit criteria met (1 pattern with countermeasures, patterns/INDEX.md, throughput_drop consults the library, maintain skill registered) and `add_playbook.md` + `playbook.md` template now authored (maintain = 6 of 9 procedures). The remaining 3 maintain procedures (add_event_type, update_aliases, deprecate_facility) are deferred by design until a real edit needs them — not a Phase 6 gap. |
...
- outcome: `follow_up_check.sh`, `countermeasure_effectiveness.sh`, `intervention_attribution.sh` (3 of 3 — complete). `follow_up_check` is **family-aware** via `--family` (tracks the metric that actually moved, e.g. exceptions/damage, not an operational proxy). `countermeasure_effectiveness` compares pre/post windows with a direction-aware IMPROVED/WORSENED verdict (`worse_direction`). `intervention_attribution` is **multi-family** (correlate-style resolver): compares the target + each `--check-variable` across an intervention date and flags MOVED confounders past `--threshold`. All three golden-tested.
...
**Golden tests:** 20 — the 17 prior (incl. `avg_cph`/`avg_damage`) + the three outcome calcs: `follow_up_check` (1888/14 = 134.86 < 138 → FAIL), `countermeasure_effectiveness` (127.00→138.50 → IMPROVED), `intervention_attribution` (cph/units STABLE at -3.55%, error_rate MOVED at +12.16% → CONFOUNDED). Expected values hand-derived from the fixtures. All passing.
...
- Investigations: 5 closed — [dal-02 throughput](data/investigations/2026-Q1/2026-03-15_dal-02_throughput_drop.md) (kaizen+a3), [chr-03 damage](data/investigations/2026-Q2/2026-04-12_chr-03_damage_spike.md) (kaizen), [ral-02](data/investigations/2026-Q2/2026-04-22_ral-02_throughput_drop.md) (kaizen), [sav-01](data/investigations/2026-Q1/2026-03-11_sav-01_throughput_drop.md) + [atl-03](data/investigations/2026-Q2/2026-04-07_atl-03_throughput_drop.md) (resolved) + [INDEX.md](data/investigations/INDEX.md); all calc-cited. The ral-02/sav-01/atl-03 trio is one mechanism → first pattern. chr-03 was produced by Codex operating the system live.
...
- Kaizens: 3 open ([dal-02 trainer-ratio](data/kaizens/open/k-2026-05-dal-02-trainer-ratio.md), [chr-03 bin-relocation](data/kaizens/open/k-2026-05-chr-03-bin-relocation.md), [ral-02 conveyor-pm](data/kaizens/open/k-2026-05-ral-02-conveyor-pm.md) — the equipment-pattern preventive countermeasure)
- Follow-ups: 11 rows in [data/follow_ups/INDEX.md](data/follow_ups/INDEX.md) (2 fired PASS, 9 pending; chr-03 rows track `damage --family exceptions`; 2 A3 rows; 3 ral-02 conveyor-PM rows tracking `conveyor_down_m --family equipment`)
...
- Maintain procedures: 6 of 9 planned — [add_calc.md](.skills/maintain/procedures/add_calc.md), [add_pattern.md](.skills/maintain/procedures/add_pattern.md), [update_pattern.md](.skills/maintain/procedures/update_pattern.md), [add_facility.md](.skills/maintain/procedures/add_facility.md), [bump_schema.md](.skills/maintain/procedures/bump_schema.md), [add_playbook.md](.skills/maintain/procedures/add_playbook.md). SKILL routes to all six; the remaining 3 (add_event_type, update_aliases, deprecate_facility) are hand-walked/deferred.
- Maintain templates: 5 — a3, kaizen, facility_profile, [pattern.md](.skills/maintain/templates/pattern.md), [playbook.md](.skills/maintain/templates/playbook.md)
- Smoke test: [verify.sh](verify.sh) — **mode-aware**: demo/unset runs the full suite; production runs the structural tier only (Sections 1/2/4/11/12/13), auto-skipping demo-scenario Sections 3/5-10. `--structural`/`--all` override. Green in both tiers (exact counts in the Session 19 working-log entry, re-verified post-rebase).
- Outcome calcs: 3 of 3 — [follow_up_check.sh](calc/outcome/follow_up_check.sh), [countermeasure_effectiveness.sh](calc/outcome/countermeasure_effectiveness.sh), [intervention_attribution.sh](calc/outcome/intervention_attribution.sh) (each golden-tested).
- Review/reporting: [.skills/review/SKILL.md](.skills/review/SKILL.md) + [status.py](.skills/review/status.py) (consistent catalog/dashboard renderer); [data/kaizens/INDEX.md](data/kaizens/INDEX.md) (catalog gap filled). Morning brief standardized in [signal-detect](.skills/signal-detect/SKILL.md).
- Onboarding workstream: [onboarding_design.md](onboarding_design.md) (design; slices 1-6 built). Built artifacts: [config/deployment.yaml.example](config/deployment.yaml.example) + [config/deployment.py](config/deployment.py) + [.skills/README.md](.skills/README.md) Step 0 (mode gate); [.skills/onboard/SKILL.md](.skills/onboard/SKILL.md) + [SETUP.md](SETUP.md) (guided flow); [.skills/onboard/reset_demo_state.py](.skills/onboard/reset_demo_state.py); [conversion/scripts/adapter_template.py](conversion/scripts/adapter_template.py)
- Skills built: signal-detect, investigate, close-loop, maintain, onboard, **review** (6; manifest in sync)
...
| 2026-05-20 | Phase 5 closeout: built the 2 remaining outcome calcs — `countermeasure_effectiveness.sh` (pre/post window comparison, direction-aware verdict) and `intervention_attribution.sh` (confounder detection across an intervention date) | The 2026-05-20 portfolio-scoping decision above, which said Phase 5 keeps `follow_up_check` as the single outcome calc unless a second "does something genuinely different" | Both clear that bar — they answer distinct questions, not count-padding. `countermeasure_effectiveness` answers "did the metric move pre→post" (handoff §13's "did the Kaizen work?" query pairs it with `follow_up_check`); `intervention_attribution` answers "is the move attributable or confounded." Each is golden-tested with an independently-derived expected value and exercised against the live dal-02 cohort scenario (recovery→IMPROVED; headcount_new the confounder, inbound_units stable). Also added the previously-missing golden test for `follow_up_check`, so all 3 outcome calcs are now locked. |
| 2026-05-20 | Phase 5 marked **complete** with the plan's "6 close-loop procedures" consolidated to 3 (`open_a3`, `open_kaizen`, `reopen_investigation`) | Plan Phase 5.4 / exit criterion "6 close-loop procedures exist" (also lists log_floor_findings, close_a3, close_kaizen) | The other three are absorbed by design: `log_floor_findings` is the SKILL's conversational intake walk (steps 2-4) backed by `intake_template.md`; `close_a3`/`close_kaizen` are driven by the outcome calcs + `follow_ups/INDEX.md` (closing = the follow-up check firing PASS, then moving the artifact to `closed/`), not standalone files; the SKILL routes `close`/`escalate` inline. Every Phase 5 *capability* is demonstrated end-to-end; the file count differs and is logged here rather than padded. |
| 2026-05-20 | Phase 6 closeout: authored `add_playbook.md` + `playbook.md` template; corrected the maintain SKILL's authored/planned split (it listed `add_facility`/`bump_schema` as "not yet authored" though both have existed since the onboarding slices) and fixed three close-loop references that called `update_pattern.md` unauthored | maintain + close-loop SKILL bodies as written | A SKILL must advertise exactly what's on disk (the project's honest-scoping discipline). Disk truth: 6 authored maintain procedures (add_calc, add_pattern, update_pattern, add_facility, bump_schema, add_playbook); 3 still planned (add_event_type, update_aliases, deprecate_facility); and `update_pattern.md` is authored, so close-loop now routes to it instead of describing a direct-edit fallback. `add_playbook.md` was authored from the existing throughput_drop/damage_spike playbooks as the worked reference. Manifest reconciled (maintain + close-loop hashes regenerated). |
...
### 2026-05-21 — Session 19 (review/reporting access layer + consistent morning brief; integrated parallel Phase 5/6 work)

- **Worked on:** the UX gap that investigations/A3s/Kaizens were "not easily shown or accessed." The data + catalogs were mostly there, but there was no *presentation/access* affordance and Kaizens had no catalog at all.
- **Built:**
  - [data/kaizens/INDEX.md](data/kaizens/INDEX.md) — the missing catalog (investigations/a3s/patterns/follow_ups had one; kaizens didn't). [open_kaizen.md](.skills/close-loop/procedures/open_kaizen.md) now maintains it (step 10 + verification + close-loop SKILL writes list).
  - [.skills/review/status.py](.skills/review/status.py) — stdlib, read-only renderer that parses every INDEX and prints one **consistent** visual (dashboard / per-category / `open` / `due`).
  - [.skills/review/SKILL.md](.skills/review/SKILL.md) — the 6th skill: "show me / list / pull up / what closed this quarter." Read-only, routes to status.py, opens named artifacts; description mutually exclusive with signal-detect (browse vs proactive scan).
  - **Morning brief standardized:** [signal-detect](.skills/signal-detect/SKILL.md) gained a fixed Output-format section (3 sections, fixed order, always shown) and reuses `status.py open|due` so the brief and the catalog views share one look — fixing the "currently varied" output.
- **Encountered:**
...
- **Verification (post-rebase, combined with the parallel Phase 5/6 work):** verify.sh Section 13. **97/97 demo, 39/39 structural; golden 20/20; reconcile clean (6 skills).** Confirmed their 3 outcome calcs and my review layer coexist.
- **Morning brief hardened + decluttered (follow-up to user requests):** standardization moved from a prose spec to a **fixed template** ([morning_brief_template.md](.skills/signal-detect/morning_brief_template.md)) + a `status.py brief` renderer that emits the OPEN/DUE sections byte-identically (no banner, for composition under the single Morning-brief banner). signal-detect reproduces the template verbatim; only NEW signals + `My read:` are model-composed. Then revised for readability on user feedback: NEW lines now show **location (id + name) · concern · dates** with **no calc command** (calcs still run for the figures; invocations live in the investigation methodology, not the glance); renderer cleaned to `·` separators, friendly dates (`May 15`), extracted facility ids, and a blank line under each header. verify Section 13e locks it.
...
### 2026-05-20 — Session 17 (Phase 5/6 closeout: outcome calcs + add_playbook)

- **Worked on:** closing out Phases 5 and 6 — the two remaining outcome calcs, golden coverage for all three, and the `add_playbook` maintain procedure.
- **Built:**
  - `calc/outcome/countermeasure_effectiveness.sh` — pre/post window means, delta + rel change, direction-aware IMPROVED/WORSENED via `worse_direction`. `calc/outcome/intervention_attribution.sh` — target vs `--check-variables` across an intervention date, flags MOVED confounders past `--threshold`, verdict LIKELY/CONFOUNDED (multi-family resolver like `correlate.sh`).
  - Golden tests for **all three** outcome calcs (`follow_up_check` had none) with hand-derived expected values; suite 17→20. `verify.sh` 6f/6g assert the live dal-02 cohort case (recovery→IMPROVED; `headcount_new` the confounder, `inbound_units` stable); demo 82→84.
  - `.skills/maintain/procedures/add_playbook.md` + `.skills/maintain/templates/playbook.md`. Synced the maintain SKILL to 6 authored procedures (it had mislisted add_facility/bump_schema as planned), fixed 3 close-loop references that called `update_pattern.md` unauthored, and reconciled the manifest.
- **Verified:** golden 20/20; `verify.sh` 84/84 (demo) and reconcile clean. New calcs staged `100755`.
- **Encountered:** the phase table's "needs a 3rd closed investigation" note was stale — 5 investigations are already closed (1 A3 + 3 Kaizens), so that criterion was already met; only the outcome calcs were genuinely outstanding. The plan's "6 close-loop procedures" is consolidated to 3 by design (logged in Decisions).
...
- **Worked on:** after a scoping conversation, reframed the build around portfolio scope, then closed the one genuine capability gap that surfaced — pattern compounding (handoff §10.3, the system's headline claim), which was completely undemonstrated.
- **Reframe (committed separately):** answered the open "how many investigations" question (polished examples over production counts), marked the change_drivers-reorder question obsolete, logged the portfolio-scoping decision (plan counts = production guidance, not portfolio acceptance criteria).
- **Pattern build (this entry):**
  - Chose the **equipment-downtime** mechanism (not cohort) for the first pattern — building a cohort pattern would need 2 more cohort dips, which would make the 2026-05-20 correlate sweep show the signature at 3 facilities and invalidate the just-opened single-facility A3. Different mechanisms keep both demonstrations honest. (Decision logged.)
  - Seeded 2 equipment-outage dips in the simulator: sav-01 (MHE drive, Mar 9-16, −12.5%) and atl-03 (conveyor gearbox, Apr 6-13, −12.7%), with quality left flat so `change_drivers` surfaces downtime — the distinguishing tell vs the cohort dip. Verified the cohort sweep is unchanged (only dal-02 negative) so the A3 holds.
  - Wrote 3 calc-grounded investigations: [ral-02](data/investigations/2026-Q2/2026-04-22_ral-02_throughput_drop.md) (full → Kaizen), [sav-01](data/investigations/2026-Q1/2026-03-11_sav-01_throughput_drop.md) + [atl-03](data/investigations/2026-Q2/2026-04-07_atl-03_throughput_drop.md) (concise → resolved — the later briefs are terser, which is itself the compounding payoff).
  - Opened [k-2026-05-ral-02-conveyor-pm](data/kaizens/open/k-2026-05-ral-02-conveyor-pm.md) (critical-spare + PM cadence; baseline-health follow-up fires PASS, conveyor_down 11.73 ≤ 60).
  - Authored [equipment_downtime_throughput_drag.md](data/patterns/equipment_downtime_throughput_drag.md) + [patterns/INDEX.md](data/patterns/INDEX.md) via `add_pattern.md` — **its first real run**. Wired [throughput_drop.md](.skills/investigate/playbooks/throughput_drop.md) Step 0 to consult the library; lifted the investigate SKILL's "patterns deferred" note; updated the add_pattern/update_pattern build-state notes.
  - verify.sh Section 10 (13 checks). **62/62**; golden 17/17; reconcile clean.
- **Encountered:**
  - The cohort-A3 vs equipment-pattern conflict (same-mechanism demonstrations pull in opposite directions on evidence breadth) was the key design insight — resolved by putting them on different mechanisms. Worth remembering for any future pattern: check whether seeding instances disturbs an existing evidence-scoped artifact.
  - This is the first time the system shows the *full* loop end-to-end: signal → investigate (×3) → recurrence recognized → pattern abstracted → playbook consults pattern on the next signal. That's the architecture's whole thesis, now demonstrable from the docs alone.
- **Next session:** portfolio is functionally complete for the throughput-drop signal. Optional: a genuinely-different 2nd outcome calc, a Phase 7 threshold-checkpoint write-up, or stop here.
