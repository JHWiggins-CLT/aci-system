...
- **Morning brief hardened (follow-up to user request):** standardization moved from a prose spec to a **fixed template** ([morning_brief_template.md](.skills/signal-detect/morning_brief_template.md)) + a `status.py brief` renderer that emits the OPEN/DUE sections byte-identically (no banner, for composition under the single Morning-brief banner). signal-detect now reproduces the template verbatim; only NEW signals + the `My read:` line are model-composed. verify Section 13e locks it.
