...
```yaml
current_phase:          "Portfolio core complete (full CI loop) AND Phases 5-6 now closed out. Phase 5: all 3 outcome calcs built with golden tests (follow_up_check, countermeasure_effectiveness, intervention_attribution); 5 closed investigations (1 A3 + 3 Kaizens). Phase 6: add_playbook procedure + playbook template authored (maintain now 6 of 9 procedures). Production-onboarding workstream complete (slices 1-6). Only optional capability modules (reports/graphing/presentations) remain."
last_completed_step:    "Phase 5/6 closeout. Authored calc/outcome/countermeasure_effectiveness.sh (pre/post window comparison, direction-aware IMPROVED/WORSENED via worse_direction) and intervention_attribution.sh (confounder detection across an intervention date, multi-family resolver). Added golden tests for all three outcome calcs — including the previously-untested follow_up_check — golden suite 17→20, expected values derived by hand. Added verify.sh 6f/6g live-scenario checks against the dal-02 cohort case (recovery→IMPROVED; headcount_new the confounder); demo 82→84. Authored .skills/maintain/procedures/add_playbook.md + .skills/maintain/templates/playbook.md; synced the maintain SKILL (now 6 authored procedures — it had mislisted add_facility/bump_schema as planned) and reconciled the manifest. verify green in both tiers."
next_concrete_action:   "Optional: capability modules (reports/graphing/presentations) via the Section 5 registry pattern — build when the first is wanted. Phases 5 and 6 are complete; the remaining maintain procedures (add_event_type, update_aliases, deprecate_facility) are deferred until a real edit needs them."
in_progress_work:       null
blocked_on:             null
last_updated:           "2026-05-20"
last_updated_by:        "session-2026-05-20-phase-5-6-closeout"
sessions_logged:        17
```
...
- Maintain procedures: 6 of 9 planned — [add_calc.md](.skills/maintain/procedures/add_calc.md), [add_pattern.md](.skills/maintain/procedures/add_pattern.md), [update_pattern.md](.skills/maintain/procedures/update_pattern.md), [add_facility.md](.skills/maintain/procedures/add_facility.md), [bump_schema.md](.skills/maintain/procedures/bump_schema.md), [add_playbook.md](.skills/maintain/procedures/add_playbook.md) (added 2026-05-20). SKILL routes to all six; the remaining 3 (add_event_type, update_aliases, deprecate_facility) are hand-walked/deferred.
- Maintain templates: 5 — a3, kaizen, facility_profile, [pattern.md](.skills/maintain/templates/pattern.md) (2026-05-20 for add_pattern), [playbook.md](.skills/maintain/templates/playbook.md) (2026-05-20 for add_playbook)
- Smoke test: [verify.sh](verify.sh) — **mode-aware**: demo/unset runs all 84 checks; production runs the 26 structural checks only (Sections 1/2/4/11/12), auto-skipping demo-scenario Sections 3/5-10. `--structural`/`--all` override. All green in both modes.
